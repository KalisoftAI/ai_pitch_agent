...
function App() {
  ...
  return (
    <div className="shell">
      ...
      <main className="main">
        <header className="topbar"><button className="icon-button menu-button" onClick={() => setMobileNav(true)} aria-label="Open menu"><Menu size={20} /></button><div><div className="eyebrow">KALISOFT AI / SALES PIPELINE</div><h1>{tab === "overview" ? "Good work starts with a clear signal." : tab === "contacts" ? "Contacts" : tab === "email" ? "Email connections" : "Signal desk"}</h1></div><div className="topbar-actions"><span className="live-dot"><i /> API healthy</span><button className="icon-button" aria-label="Settings"><Settings2 size={18} /></button></div></header>
...
function Contacts({ onNotice }) {
  const [contacts, setContacts] = useState([]); const [query, setQuery] = useState(""); const [stats, setStats] = useState(null); const [showForm, setShowForm] = useState(false); const [busy, setBusy] = useState(false);
